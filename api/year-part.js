...
module.exports=async function(req,res){
  ...
  const scanMode=String(req.query.scan||"fast");

  // V44:
  // fast = foreground search, only 4 probes/month
  // deep = optional background fill, complementary dates
  const fastDays=[1,9,17,25];
  const deepDays=[3,5,7,11,13,15,19,21,23,27,29,31];
  const scanDays=scanMode==="deep" ? deepDays : fastDays;
  const hits=[];
    ...
    for(const month of quarter){
      ...
      for(let i=0;i<days.length;i+=3){
        ...
        const got=await Promise.all(batch.map(day=>{
          ...
          return availability(target,`${year}${mm}${dd}`,scanMode==="deep"?4200:3300);
        }));
        hits.push(...got.filter(Boolean));
        await sleep(scanMode==="deep"?110:70);
      }
    }
  }

  await runDays(scanDays);
  ...
  if(result.found){
    // part cache
    await cacheSet(cfg,`ytm:v44:${scanMode}:${username.toLowerCase()}:${type}:${year}:${part}`,result,7776000);
